...
obis_clean <- obis_clean %>%
  mutate(zone = case_when(
    ...
    bathymetry >= 200 & minimumDepthInMeters > 200 & bath_obs_prop_min < 0.99 ~ "deep (midwater)",
    bathymetry >= 200 & minimumDepthInMeters > 200 & bath_obs_prop_min >= 0.99 ~ "deep (benthic)"
  ))
```
...
r <- rotate(raster_stack)
terra::plot(log(r,10), col = colors)
```

```{r}
# Reproject raster to Robinson projection
rob_proj <- "+proj=robin"
raster_stack_robin <- project(raster_stack, rob_proj)

# Get world map in Robinson projection
world <- ne_countries(scale = "medium", returnclass = "sf")
world_robin <- st_transform(world, crs = rob_proj)

# Convert raster to data frame for ggplot
raster_df <- as.data.frame(raster_stack_robin[[1]], xy = TRUE)
colnames(raster_df) <- c("x", "y", "observations")  # Rename columns

# Define color palette
colors <- met.brewer(name = "Isfahan1", n = 20, type = "continuous")

# Plot with ggplot
ggplot() +
  geom_raster(data = raster_df, aes(x = x, y = y, fill = log10(observations))) +
  geom_sf(data = world_robin, fill = "grey", color = NA) +
  scale_fill_gradientn(colors = colors, name = "log10(observations)") +
...
```{r}
# Reproject raster stack to Robinson projection
rob_proj <- "+proj=robin"
raster_stack_robin <- project(raster_stack, rob_proj)
...
# Define color palette
colors <- met.brewer(name = "Hokusai2", n = 20, type = "continuous")
...
for (i in 1:nlyr(raster_stack_robin)) {
  ...
  plot <- ggplot() +
    ...
    geom_sf(data = world_robin, fill = "grey", color = NA) +
    scale_fill_gradientn(colors = colors, name = "log10(observations)", limits = color_limits) +  # Set colorbar limits
    coord_sf(crs = rob_proj) +
    theme_minimal() +
    theme(
      axis.title.x = element_blank(),
